...
const KanbanBoardPage = () => {
	...
	const moveColumnToEnd = async () => {
		try {
			await axios.patch(`/kanban/status/${selectedColumn.id}/move-to-end`);
			showNotification("Статус перемещен в конец списка", "success");
			fetchStatus();
			closeColumnDrawer();
		} catch (error) {
			console.error("Error moving column to end:", error);
			showNotification("Ошибка при перемещении статуса", "error");
		}
	};
